...
public class BillService {
	...
	public ArrayList<Bill> getBills() {
		...
		try {
			...
			while(result.next()) {
				...
				bills.add(new Bill(result.getInt("CustomerId"), result.getString("BillNumber"), result.getString("Prev_Reading"),
	...
	public Object insertBill(Bill bill) {
		String cmd = "insert into bill(CustomerId,BillNumber,Prev_Reading,Billing_Date,Due_Amount,Current_Reading,Bill_Amount) values (?,?,?,?,?,?,?)"; 
	...
	public Object updateBill(Bill bill) {
		String cmd = "update bill set  BillNumber= ?,Prev_Reading= ?,Billing_Date= ?,Due_Amount= ?, CustomerId= ?,Current_Reading= ?,Bill_Amount= ? where bill_id= ?"; 
		...
		try {
			...
			ps.setString(3, bill.getBILLING_DATE());
			ps.setString(4, bill.getDUE_AMOUNT());
			ps.setInt(5, bill.getCustomerId());
			ps.setString(6, bill.getCURRENT_READING());
			ps.setString(7, bill.getBILL_AMOUNT());
